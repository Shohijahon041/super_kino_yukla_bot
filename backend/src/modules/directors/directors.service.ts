...
export class DirectorsService {
  ...
  async findBySlug(slug: string) {
    ...
    if (!person) {
      throw new NotFoundException(`Director with slug "${slug}" not found`);
    }
  ...
  async create(dto: CreatePersonDto) {
    ...
    if (existing) {
      throw new ConflictException('Director with this slug already exists');
    }
  ...
  async delete(id: string) {
    ...
    const hasRelations =
      person._count.movieDirector > 0 || person._count.seriesDirector > 0;

    if (hasRelations) {
      throw new BadRequestException(
        'Cannot delete director with existing movie/series relations. Remove all relations first.',
      );
    }
  ...
  async getFilmography(id: string) {
    ...
    const [movieRoles, seriesRoles] = await Promise.all([
    ...
    return {
      ...
      filmography: {
        directedMovies: movieRoles.map((r) => r.movie),
        directedSeries: seriesRoles.map((r) => r.series),
      },
      totalCredits: movieRoles.length + seriesRoles.length,
    };
  }
